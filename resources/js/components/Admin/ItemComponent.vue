<template>
            ...
            <div class="dropdown">
              ...
              <ul class="dropdown-menu rounded-4" aria-labelledby="dropdownMenuButton">
                ...
                <li>
                  ...
                  <button class="dropdown-item text-info" data-bs-target="#editItemModal" data-bs-toggle="modal" v-on:click="openEditModal(slotProps.data)">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="hero-icon me-2">
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
    ...
    <div class="modal-dialog modal-fullscreen-md-down modal-lg modal-dialog-scrollable">
      <form @submit.prevent="storeItem" class="modal-content rounded-4">
        ...
        <div class="modal-body bg-body">
          ...
          <!-- Add Extra Fields -->
          <div v-for="(productField, index) in variants" :key="index">
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              <div class="text-center">
                <label
                  :for="'input-variant-image-' + index"
                  class="cursor-pointer position-relative rounded-4"
                  :class="{
                    'border border-dark': !variantImageUrl[index]
                  }"
                  style="width: 160px; height: 160px"
                >
                  <img
                    :src="variantImageUrl[index]"
                    class="border border-dark rounded-4"
                    alt="placeholder-image"
                    v-if="variantImageUrl[index]"
                    style="width: 160px; height: 160px; object-fit: contain"
                  />
                  <div class="position-absolute top-50 start-50 translate-middle" v-if="!variantImageUrl[index]">
                    <div class="">
                      <svg xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48">
                        <path
                          d="M180 936q-24.75 0-42.375-17.625T120 876V276q0-24.75 17.625-42.375T180 216h409v60H180v600h600V468h60v408q0 24.75-17.625 42.375T780 936H180Zm520-498v-81h-81v-60h81v-81h60v81h81v60h-81v81h-60ZM240 774h480L576 582 449 749l-94-124-115 149Zm-60-498v600-600Z"
                ...
                <input
                  :id="'input-variant-image-' + index"
                  class="d-none"
                  type="file"
                  name="input-variant-image"
              ...
              <div class="row">
                ...
                <div class="col-md-6 mb-3">
                  <div class="row mb-1 small border-bottom">
                    <div class="col-6 text-muted">
                      <div>Margin</div>
                      <div class="fw-bold">{{ percentageFormat(calculateMargin(productField)) }}</div>
                    </div>
                    <div class="col-6 text-muted">
                      <div>Profit</div>
                      <div class="fw-bold">{{ formatNumber(calculateProfit(productField)) }}</div>
                    </div>
                  </div>
                </div>
                ...
                <div class="col-md-6 mb-3">
                  ...
                  <div class="form-text" v-if="productField.discount > 0">
                    Price after Discount: <span class="fw-bold">{{ formatNumber(calculatePriceAfterDiscount(productField)) }}</span>
                  </div>
                </div>
            ...
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              ...
              <div class="row">
                <div class="col-md-12 mb-3">
                  <label for="code" class="form-label fw-bold"> Product Code (the unique identifier and digital fingerprint of product) </label>
                  <input type="text" class="form-control" v-model="productField.code" id="code" name="code" />
                </div>
                <div class="col-md-6 mb-3">
                  <label for="sku" class="form-label fw-bold">SKU Stock keeping unit </label>
                  <input type="text" class="form-control" v-model="productField.sku" id="sku" name="sku" />
                </div>
                <div class="col-md-6 mb-3">
                  <label for="barcode" class="form-label fw-bold">Barcode (ISBN, UPC, GTIN, etc.)</label>
                  <input type="text" class="form-control" v-model="productField.barcode" id="barcode" name="barcode" />
                </div>
              </div>
              <!-- <div class="form-check mb-3">
                <input class="form-check-input" type="checkbox" v-model="data.track_stock" id="trackStockCheckbox" />
            ...
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              <div class="col-md-6 mb-3">
                ...
                <input type="text" class="form-control" v-model="productField.color" id="color" name="color" />
              </div>
              <div class="col-md-6 mb-3">
                <label for="size" class="form-label fw-bold">Size</label>
                <input type="text" class="form-control" v-model="productField.size" id="size" name="size" />
              </div>
            </div>
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              <div class="form-label fw-bold mb-3">Media</div>
              <div class="mb-3">
                ...
                <FileUpload
                  name="images[]"
                  :fileLimit="10"
                  @select="handleVariantAdditionalImage(index, $event)"
                  :multiple="true"
                  :maxFileSize="2097152"
                  :showCancelButton="false"
                  :showUploadButton="false"
                  @remove="handleImageRemove($event)"
                />
              </div>
            </div>
          </div>
          ...
          <div class="d-flex align-items-center mb-3">
            <button type="button" class="btn btn-primary m-3 ms-auto px-md-4" @click="handleReplicateFields">Add Variants</button>
          </div>
  ...
  <!-- MODAL EDIT -->
  <div class="modal" id="editItemModal" tabindex="-1" aria-labelledby="editItemModalLabel">
    <div class="modal-dialog modal-fullscreen-md-down modal-lg modal-dialog-scrollable">
      <form @submit.prevent="updateItem(editData.id)" class="modal-content rounded-4">
        <div class="modal-header d-flex align-items-center">
          ...
          <div>
            ...
            <button type="submit" class="btn btn-primary px-md-4" data-bs-dismiss="modal" :disabled="loading">Save</button>
          </div>
        </div>
        <div class="modal-body bg-body">
          <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
          ...
          <!-- Add Extra Fields -->
          <div v-for="(productField, indexItem) in editData.variants" :key="indexItem">
            <div class="d-flex align-items-center mb-3">
              <button
                v-if="editData.variants.length > 0"
                type="button"
                class="btn btn-danger ms-auto px-md-4"
                @click="handleDeleteEditVairantField(productField.id, indexItem)"
              >
                X
              </button>
            </div>
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              <div
                class="text-center"
                :class="{
                  ' mb-3': productField.image_path
                }"
              >
                <label
                  :for="'input-image-edit-variant' + indexItem"
                  class="cursor-pointer position-relative rounded-4"
                  :class="{
                    'border border-dark': !productField.image_url
                  }"
                  style="width: 160px; height: 160px"
                >
                  <img
                    :src="productField.image_url"
                    class="border border-dark rounded-4"
                    alt="placeholder-image"
                    v-if="productField.image_url"
                    style="width: 160px; height: 160px; object-fit: contain"
                  />
                  <div class="position-absolute top-50 start-50 translate-middle" v-if="!productField.image_url">
                    <div class="">
                      <svg xmlns="http://www.w3.org/2000/svg" height="48" viewBox="0 96 960 960" width="48">
                        <path
                          d="M180 936q-24.75 0-42.375-17.625T120 876V276q0-24.75 17.625-42.375T180 216h409v60H180v600h600V468h60v408q0 24.75-17.625 42.375T780 936H180Zm520-498v-81h-81v-60h81v-81h60v81h81v60h-81v81h-60ZM240 774h480L576 582 449 749l-94-124-115 149Zm-60-498v600-600Z"
                ...
                <input
                  :id="'input-image-edit-variant' + indexItem"
                  class="d-none"
                  type="file"
                  :name="'input-image-edit-variant' + indexItem"
                  accept="image/x-png, image/jpeg, image/jpg"
                  value=""
                  @change="handleVariantEditImage($event, indexItem)"
                />
              </div>
              <div class="d-flex align-items-center justify-content-center">
                <button
                  ...
                  @click="handleDeleteVariantEditImage(productField.id, indexItem)"
                  v-if="productField.image_url"
                >
                  Delete Image
                </button>
            ...
            <div class="border rounded-4 p-3 mb-3 bg-white border-light shadow-sm">
              ...
              <div class="mb-3">
                ...
                <FileUpload
                  name="imagesEdit[]"
                  :fileLimit="editData.variants?.additional_images?.length ? 10 - editData.variants?.additional_images?.length : 10"
                  @select="handleEditImageSelect($event, indexItem)"
                  :multiple="true"
                  :maxFileSize="2097152"
                  :showCancelButton="false"
                  :showUploadButton="false"
                  @remove="handleEditImageRemove($event, indexItem)"
                  :ref="'fileuplaod' + indexItem"
                />
              </div>
              <div class="mb-3">
                <table class="table">
                  <tbody>
                    <tr v-for="variantImage in productField.additional_images" :key="variantImage.id">
                      <th>
                        <img :src="variantImage?.thumbnail_url" width="50" />
                      </th>
                      <td>
                        <button
                          type="button"
                          class="btn btn-sm btn-danger shadow-none"
                          @click="deleteEditAdditionalImage(variantImage)"
                          :disabled="loading"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="hero-icon">
                            <path
          ...
          <!--End Add Extra Fields -->
          <div class="d-flex align-items-center mb-3">
            <button type="button" class="btn btn-primary m-3 ms-auto px-md-4" @click="handleEditAddFields">Add Variants</button>
          </div>
          <div class="border rounded-4 p-3 bg-white border-light shadow-sm">
            <div class="form-label fw-bold mb-3">Search engine listing preview</div>
            <div class="mb-3">
...
export default {
  ...
  methods: {
    /**
     * Function to replicate the fields
     */
    handleReplicateFields() {
      this.variants.push({
        id: null,
        image: '',
        cost: null,
    ...
    /**
     * Function to add fields in edit modal
     */
    handleEditAddFields() {
    ...
    /**
     * Function to delete the extra fields in edit modal
     */
    handleDeleteEditVairantField(id, index) {
      if (id == null) {
        this.editData.variants.splice(index, 1);
        return;
      }
      this.deleteItem(id);
      this.editData.variants.splice(index, 1);
    ...
    storeItem() {
      ...
      if (this.variants.length) {
        this.variants.forEach((variant, variantIndex) => {
          ...
          formData.append(`variants[${variantIndex}][cost]`, variant.cost);
          formData.append(`variants[${variantIndex}][discount]`, variant.discount);
          formData.append(`variants[${variantIndex}][price]`, variant.price);
          formData.append(`variants[${variantIndex}][code]`, variant.code);
          formData.append(`variants[${variantIndex}][sku]`, variant.sku);
          formData.append(`variants[${variantIndex}][barcode]`, variant.barcode);
          formData.append(`variants[${variantIndex}][in_stock]`, variant.in_stock);
          formData.append(`variants[${variantIndex}][color]`, variant.color);
          formData.append(`variants[${variantIndex}][size]`, variant.size);

          if (variant.image) {
            formData.append(`variants[${variantIndex}][image]`, variant.image);
          }

          if (variant.additional_images.length) {
            variant.additional_images.forEach((additionalImage, imageIndex) => {
              formData.append(`variants[${variantIndex}][additional_images][${imageIndex}]`, additionalImage);
            });
          }
        });
      }
      ...
      axios
        ...
        .catch(({ response }) => {
          if (response && (response.status === 422 || response.status === 429)) {
    ...
    updateItem(id) {
      const _self = this;
      this.editData?.variants?.forEach((element, index) => {
        _self.$refs['fileuplaod' + index][0].clear();
      });
      this.errors = {};
      topbar.show();
      this.loading = true;
      ...
      if (this.editData.variants.length) {
        this.editData.variants.forEach((variant, variantIndex) => {
          formData.append(`variants[${variantIndex}][id]`, variant.id);
          formData.append(`variants[${variantIndex}][cost]`, variant.cost);
          formData.append(`variants[${variantIndex}][discount]`, variant.discount);
          formData.append(`variants[${variantIndex}][price]`, variant.price);
        ...
        if (this.editItemAdditionalImages[variantIndex] && this.editItemAdditionalImages[variantIndex].length) {
                this.editItemAdditionalImages[variantIndex].forEach((additionalImage, imageIndex) => {
                    formData.append(`variants[${variantIndex}][additional_images][]`, additionalImage);
                });
            }
        });
      }
    ...
    /**
     * Function to handle product when open edit
      ...
      if (item?.variants.length > 0) {
        item.variants.forEach((el, index) => {
          this.editData.variants.push({
            ...
            color: el.color,
            size: el.size,
            additional_images: el.additional_images
          });
        //   this.editItemAdditionalImages[index] =[]
        //   this.editItemAdditionalImages[index].push(el.additional_images);
        });
        // item.variants.forEach(el => {
        //   el.additional_images.forEach(el => {
        //     // this.editItemAdditionalImages.push(el);
        //   });
        // });
      }
    },
    ...
    handleDeleteVariantEditImage(id, index) {
      if (id == null) {
      ...
      Swal.fire(swalConfig()).then(result => {
        if (result.value) {
          topbar.show();
          axios
            .delete(`/admin/items/image/${id}`)
            .then(response => {
              if (response.status == 200) {
                this.editData.variants[index].image = '';
                this.editData.variants[index].image_url = null;
    ...
    /**
     * Function to delete the image when changed
     * @param index get index
     */
    onChangeDeleteVariantEditImage(index) {
      this.editData.variants[index].image = null;
      this.editData.variants[index].image_url = null;
      this.editData.variants[index].image_path = null;
      this.$toast.success('Image has been deleted');
    },
    ...
    /***
     * Function to delete the image inside the edit modal
     */
    deleteEditAdditionalImage(image) {
      Swal.fire(swalConfig()).then(result => {
        if (result.value) {
          topbar.show();
          this.loading = true;
          axios
            .delete(`/admin/items/additional-image/${image.id}`)
            .then(response => {
              var newAdditionalVariantImage = this.editItemAdditionalImages.filter(i => i !== image);
              this.editItemAdditionalImages = newAdditionalVariantImage;
              this.fetchItems();
              this.$toast.success('Product additional image has been Deleted');
            })
            .catch(error => console.log(error))
    ...
    /**
     * Function to handle variant additonal images
     */
    handleVariantAdditionalImage(index, event) {
      const files = Array.from(event.files);
      for (var i = 0; i < files.length; i++) {
        this.variants[index].additional_images.push(files[i]);
      }
    },
    ...
    /**
     * Function to upload additional images on edit modal
     */
    handleEditImageSelect(event, index) {
      var files = event.files;
      if (!files.length) {
        return false;
      }
      this.editItemAdditionalImages[index] = []
      for (var i = 0; i < files.length; i++) {
        this.editItemAdditionalImages[index].push(files[i]);
      }
    },

    /**
     *  Function to remove the additional image in variant create modal
     * @param event get file
     */
    handleImageRemove(event) {
      this.variants.additional_images = [];
      var files = event.files;
      for (var i = 0; i < files.length; i++) {
        this.variants.additional_images.push(files[i]);
      }
    },

    /**
     *  Function to remove and add the file in variant additional images
     * @param event get file
     * @param index get index
     */
    handleEditImageRemove(event, index) {
      this.editData.variants[index].additional_images = [];
      var files = event.files;
      for (var i = 0; i < files.length; i++) {
        this.editItemAdditionalImages[index].push(files[i]);
      }
    },
    // removeImage(fieldIndex, imageIndex) {
    //   const images = this.variants[fieldIndex].additional_images.slice();
    //   images.splice(imageIndex, 1);
    //   this.$set(this.variants, fieldIndex, {
    //     ...this.variants[fieldIndex],
    ...
    /**
     * Function to handle variant extra field main image
     */
    handleVariantImage(event, index) {
      var file = event.target.files[0];
      if (file) {
        this.variants[index].image = file;
        this.variantImageUrl[index] = [];
        this.variantImageUrl[index].push(URL.createObjectURL(file));
      } else {
        // this.data.image = this.data.image;
        this.variantImageUrl = this.variantImageUrl;
      }
    },
    openImageEdit(event) {
      var file = event.target.files[0];
    ...
    /**
     *  Function to show the image in edit modal
     * @param event File property
     * @param index index of file
     */
    handleVariantEditImage(event, index) {
      var file = event.target.files[0];
      if (file) {
        this.editData.variants[index].image = file;
        this.editData.variants[index].image_url = URL.createObjectURL(this.editData.variants[index].image);
      } else {
        this.editData.image = this.editData.image;
        this.editData.image_url = this.editData.image_url;
      }
    ...
    calculatePriceAfterDiscount(productField) {
      if (productField.discount > 0) {
        return productField.price - (productField.discount / 100) * productField.price;
      }
      return productField.price;
    },
    calculateProfit(productField) {
      return this.calculatePriceAfterDiscount(productField) - productField.cost;
    },
    calculateMargin(productField) {
      const profit = this.calculateProfit(productField);
      const priceAfterDiscount = this.calculatePriceAfterDiscount(productField);
      if (priceAfterDiscount === 0) return 0;
      return (profit / priceAfterDiscount) * 100;
    }
  },
