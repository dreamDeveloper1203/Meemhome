...
class PointOfSale extends Component<Props, State> {
    ...
    getCategories = (): void => {
        httpService
            .get(`https://meemhome.com/api/v1/categories/items/pos?access_token=$2y$10$ZUGq7/54qtET9J4C2LzIx.zdKvCMNNzpc4iziweN8CoXoCW6zTXCS`)
            .then((response: any) => {
    ...
    calculateTotal = (): void => {
        ...
        if (this.state.cart.length > 0) {
            this.state.cart.map((item: ICartItem) => {
                ...
                _subtotal += (item.cost || 0) * (item.quantity || 0);
                // _subtotal += ((this.state.currentPrice === 0 ? item.retail_price : item.wholesale_price) || 0) * (item.quantity || 0);
            });
        }
    ...
    addToCart = (product: IProduct): void => {
        ...
        let cartItem: ICartItem = {
            ...
            super_dealer_price: product.super_dealer_price,
    ...
    allProducts = (): IProduct[] => {
        ...
        this.state.categories.map((category: ICategory) => {
            category.items.map((product: IProduct) => {
    ...
    handleSelectRadio = (index: number): void => {
        const cart = this.state.cart.map((item: ICartItem) => {
            let cartItem: ICartItem = {
                ...
                cost: ((index === 0) ? item.retail_price : item.wholesale_price),
                ...
                quantity: 1,
                super_dealer_price: item.super_dealer_price,
                            ...
                            <div className="card-header bg-white border-bottom-0 p-0">
                                <form onSubmit={event => this.handleSearchSubmit(event)}>
                                    <input
                                        ...
                                        name="search"
                                        id="barcode-input"
                                        placeholder={t('Scan barcode or search', 'امسح الباركود ضوئيًا أو ابحث بالاسم أو SKU')}
                                                    ...
                                                    return (
                                                        <tr key={item.cartId}>
                                                            <td width={300}>
                                                                <div className="d-flex mb-2">
                                                                    <div className="d-flex flex-grow-1">
                                                                        ...
                                                                        <div>
                                                                            <div className="fw-bold">{item.search_name}</div>
                                                            ...
                                                            <td width={150} className="text-center align-middle">
                                                                {this.currencyFormatValue((item.quantity || 0) * (item.cost || 0))}
                                                            </td>
                                                        </tr>
                                                    );
                                                })}
                                                                ...
                                                                <div key={product.id} className="col-lg-4 col-md-4 col-sm-6 col-6 mb-0 p-0">
                                                                    <div
                                                                        ...
                                                                        <div className="position-absolute bottom-0 start-0 h-100 d-flex flex-column align-items-center justify-content-center p-4 mb-0 w-100 cell-item-label text-center">
                                                                            ...
                                                                            <div className="fw-normal">$ {product.cost || 0}</div>
                                                                            <div className="product-name" dir="auto"> {product.in_stock} </div>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            )}
