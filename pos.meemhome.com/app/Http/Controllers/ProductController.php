...
class ProductController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        $request->validate([
            ...
            'variants.*.retailsale_price' => ['nullable', 'numeric', 'min:0'],
            'variants.*.wholesale_price' => ['nullable', 'numeric', 'min:0'],
            'variants.*.in_stock' => ['nullable', 'numeric'],
            'variants.*.retail_barcode' => ['nullable', 'string', 'max:43'],
            'variants.*.retail_sku' => ['nullable', 'string', 'max:64'],
            'variants.*.wholesale_barcode' => ['nullable', 'string', 'max:43'],
            'variants.*.wholesale_sku' => ['nullable', 'string', 'max:64'],
            'variants.*.color' => ['required', 'string', 'max:200'],
            'variants.*.size' => ['required', 'string', 'max:200'],
        ...
        if ($request->has('variants')) {
            foreach ($request->variants as $_variant) {
                $variant = $product->variants()->updateOrCreate(
                    ['id' => $_variant['id'] ?? null],
                    [
                        'name' => $request->name . ' + ' . $_variant['color'] . ' + ' . $_variant['size'],
                        'category_id' => $request->category,
                        'cost' => $_variant['cost'] ?? 0,
                        'retailsale_price' => $_variant['retailsale_price'] ?? 0,
                        'wholesale_price' => $_variant['wholesale_price'] ?? 0,
                        'in_stock' => $_variant['in_stock'] ?? 0,
                        'retail_barcode' => $_variant['retail_barcode'],
                        'wholesale_barcode' => $_variant['wholesale_barcode'],
                        'retail_sku' => $_variant['retail_sku'] ?? null,
                        'wholesale_sku' => $_variant['wholesale_sku'] ?? null,
                        'color' => $_variant['color'],
                        'size' => $_variant['size'],
                        'track_stock' =>  array_key_exists('track_stock', $_variant) ? true : false,
                        'continue_selling_when_out_of_stock' => array_key_exists('continue_selling_when_out_of_stock', $_variant) ? true : false,
                    ]
                );

                if (isset($_variant['image'])) {
                    $variant->updateImage($_variant['image']);
                }
            }
        }
